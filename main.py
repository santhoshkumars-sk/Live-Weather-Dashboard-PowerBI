...
# Function to Get an API Key with Rate Handling
def get_api_key():
    for _ in range(len(api_keys)):
        api_key = next(api_key_cycle)
        if key_usage[api_key] < 57:
            return api_key
    print("[⚠️] All API keys exhausted. Waiting for reset...")
    time.sleep(60)  # Wait for API limit reset
...
def fetch_data(lat, lon, city):
    for _ in range(3):  # Retries
        ...
        try:
            ...
            if weather_response.status_code != 200 or pollution_response.status_code != 200:
                print(f"[⚠️] API error for {city}: Weather={weather_response.status_code}, Pollution={pollution_response.status_code}")
                continue
            ...
            if "main" in weather_data and "list" in pollution_data:
                key_usage[api_key] += 2  # Update API key usage
                timezone_offset = weather_data["timezone"]  

                # Get pollutant data safely
                pollutants = pollution_data["list"][0].get("components", {})

                # Correct Last Updated Time (Local Time)
    ...
    print(f"[⚠️] Skipping {city} due to repeated failures.")
    return None
...
def fetch_all_data():
    ...
    print(f"[✅] Successfully updated {len(data_df)} records in Google Sheets.")
